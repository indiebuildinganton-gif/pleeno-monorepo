/**
 * Agency Zone Middleware for Authentication
 *
 * This middleware:
 ...
 * Access the agency zone through the shell app at http://localhost:3005/agency
...
export async function middleware(request: NextRequest) {
  ...
  const isApiRoute = request.nextUrl.pathname.startsWith('/agency/api')
  ...
  if (!isApiRoute && !user) {
    ...
    const originalPath = request.nextUrl.pathname.replace(/^\/agency/, '') || '/'
    redirectUrl.searchParams.set('redirectTo', `/agency${originalPath}`)
...
export const config = {
  matcher: [
    '/agency/((?!api|_next/static|_next/image|favicon.ico|.*\\.(?:svg|png|jpg|jpeg|gif|webp)$).*)',
  ],
}
